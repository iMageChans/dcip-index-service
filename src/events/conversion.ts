...
export async function ConversionEvent(ctx: ProcessorContext<Store>): Promise<Conversion[]>{
    let transfers: Conversion[] = []
    for (let block of ctx.blocks) {
        for (let event of block.events) {
            ...
            if (decoded.__kind === "D9ToUSDTConversion" || decoded.__kind === "USDTToD9Conversion") {
                const commonData = {
                    event_id: event.id,
